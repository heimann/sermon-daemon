...
pub const Storage = struct {
    ...
    fn initWithMode(allocator: std.mem.Allocator, db_path: []const u8, read_only: bool, memory_limit_mb: u32) !Storage {
        ...
        // Bound the buffer pool: DuckDB otherwise defaults to ~80% of system
        // RAM and grows unboundedly under the daemon's persistent connection.
    ...
    pub fn reconnect(self: *Storage) !void {
        ...
        // Init schema on the new connection before committing it to self: on
        // failure the errdefers close new_conn/new_db and self stays null
        // from the close above, so deinit() remains a safe no-op.
        if (!self.read_only) {
    ...
    fn initSchema(conn: c.duckdb_connection) !void {
        ...
        var result: c.duckdb_result = undefined;
        const state = c.duckdb_query(conn, schema_sql.ptr, &result);
        defer c.duckdb_destroy_result(&result);
    ...
    pub fn getProcesses(self: *Storage, timestamp: ?i64) ![]ProcessInfo {
        ...
        while (i < row_count) : (i += 1) {
            ...
            procs[i] = ProcessInfo{
                ...
                .threads = threads,
                .username = username,
                // Per-process I/O + cgroup/unit are push-only (not persisted in
                // the local DuckDB - keeps the daemon's RSS at baseline). Rows
                // read back from storage default them; live collection fills the
                // real values for the push payload (see collector.enrichProcess).
                .io_read_bytes = 0,
                .io_write_bytes = 0,
                .cgroup = try self.allocator.dupe(u8, ""),
...
test "Storage: failed reconnect stays safe to deinit" {
    ...
    // After a failed reconnect the handles are already closed; deinit() (the
    // defer above) must stay a safe no-op and not double-free.
